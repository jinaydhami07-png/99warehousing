# Buy Per Square Foot (BPSF) — Project Handoff
...
The project is divided in two at the top level:

```
buypersquarefoot/
  deploy/            ← the app. Push this to GitHub / host this.
  ...
  project-files/     ← everything else, not needed to run anything
    ...
    local-tools/       Windows startup scripts, old standalone seeder
```
...
public/
  ...
  assets/
    ...
    bpsf-demo.js    <- offline fallback backend
    img/hero.png
```
...
All secrets live in **`deploy/server/.env`**. That file is real and populated. `deploy/server/.env.example`
is the committed template with placeholders.

| Line | Key | Status |
|---|---|---|
| 2 | `PORT` | `5000` |
| 8 | `MONGODB_URI` | **live Atlas connection string** |
| 13 | `CORS_ORIGINS` | allowed origins |
| 17 | `JWT_ACCESS_SECRET` | set |
| 18 | `JWT_REFRESH_SECRET` | set |
| 27 | `ADMIN_PASSKEY` | **`2010J`** |

### Admin access

1. Open `http://localhost:5000/login.html`
2. Scroll to the bottom, click **"Staff access ▾"**
3. Enter **`2010J`** → lands on `dashboard.html`

There is no admin link in the public navigation, by design. A normal user listing a property
never sees the dashboard.

### Keys NOT yet configured

Neither is wired up; both degrade gracefully rather than crashing.

- **Google OAuth** — `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` do not exist in `.env` yet.
  `GET /api/v1/auth/google` currently redirects to `/login.html?error=google_not_configured`.
  To enable: add both keys to `deploy/server/.env`, then implement the callback in
  `deploy/server/src/controllers/auth.controller.js` (the stub `googleStart` is at line ~35).
- **Cloudinary** (image hosting) — no keys, no upload endpoint. See §5.
