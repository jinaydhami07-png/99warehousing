/* ============================================================
   Buy Per Square Foot — offline demo backend  (CLIENT SIDE)
   ------------------------------------------------------------
...
(function (global) {
  ...
  function adminUser() {
    return { id: 'demo-admin', name: 'BPSF Admin', email: 'admin@buypersqft.local', role: 'admin', isVerified: true, authProvider: 'local' };
  }
  ...
  function audit(action, entityId, before, after) {
    ...
    rows.unshift({
      ...
      actorEmail: (s && s.email) || 'admin@buypersqft.local',
    ...
    if (path.indexOf('/admin/') === 0) {
      ...
      if (method === 'POST' && path === '/admin/properties') {
        ...
        var rec2 = {
          ...
          description: body.description || '',
          ownerName: body.ownerName || 'Added by admin', ownerEmail: 'admin@buypersqft.local',
  ...
  var engaged = false;
  function showBanner() {
    if (document.getElementById('demoBanner') || sessionStorage.getItem('bpsf.demo.banner_dismissed')) return;
    var bar = document.createElement('div');
    bar.id = 'demoBanner';
    bar.setAttribute('role', 'status');
    /* Be blunt about the one thing that actually surprises people: changes
       made here never reach the database. The previous copy said everything
       was "fully functional", which read as "this is saving" — an admin
       approved a listing, saw the counters move, and reasonably assumed the
       server had it. It did not. */
    bar.innerHTML =
      '<span class="demo-dot"></span>' +
      '<span><strong>Demo mode — nothing is saved to the server.</strong> ' +
      'No backend is reachable, so listings, sign-in and approvals all run on sample data ' +
      'held in this browser only. Refreshing keeps it; another device or browser will not see it.</span>' +
      '<button type="button" aria-label="Dismiss">✕</button>';
    bar.querySelector('button').onclick = function () {
      try { sessionStorage.setItem('bpsf.demo.banner_dismissed', '1'); } catch (e) {}
      bar.remove();
    };
